...
import com.douglas.thecarserviceapp.adapter.SearchServiceProviderAdapter;
...
public class SearchServiceProvider extends AppCompatActivity {
    ...
    SearchServiceProviderAdapter searchServiceProviderAdapter;
    User user;
    List<User> providers;
    ...
    protected void createEvents() {
        btnSearchByCity.setOnClickListener(v -> {
            if (user != null) {
                ...
                providers = dbHelper.getServiceProvidersByCity(city);
                if (providers.isEmpty()) {
                    Toast.makeText(this, "0 Results", Toast.LENGTH_SHORT).show();
                } else {
                    recyclerView.setLayoutManager(new GridLayoutManager(this, 1));
                    recyclerView.setAdapter(new SearchServiceProviderAdapter(providers));
                }
            }
        });
    }
